...
class ArxivHTMLToMarkdown:
    ...
    def process_paragraph(self, para_elem) -> str:
        ...
        bold_elements.extend(para_elem.find_all(class_=re.compile('ltx_font_bold')))
        ...
        italic_elements.extend(para_elem.find_all(class_=re.compile('ltx_font_italic')))
        ...
        code_elements.extend(para_elem.find_all(class_=re.compile('ltx_font_typewriter')))
    ...
    def process_figure(self, figure_elem) -> str:
        ...
        if caption:
            ...
            if fig_tag:
                ...
                result.append(f"**Figure {self.figure_counter}:** {caption_text}")
        else:
    ...
    def process_section(self, section_elem, level: int = 1) -> None:
        ...
        for child in section_elem.children:
            ...
            class_names = child.get('class', []) if hasattr(child, 'get') else []
    ...
    def convert(self, html_content: str) -> str:
        ...
        if article:
            # Process all sections (including appendices)
            sections = article.find_all('section', recursive=False)
            for section in sections:
                # Check if it's an appendix
                if 'ltx_appendix' in section.get('class', []):
                    # Process appendix with level 1
                    self.process_section(section, level=1)
                elif 'ltx_section' in section.get('class', []):
                    self.process_section(section, level=1)
        ...
        markdown = re.sub(r'\[([^\]]+)\]\(([^)\s]+)\)', replace_url_backticks, markdown)
